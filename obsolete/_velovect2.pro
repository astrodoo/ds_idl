; $Id: //depot/idl/IDL_63_RELEASE/idldir/lib/velovect.pro#1 $
;
    ...
    y_b1=y1+y_step
        if (not keyword_set(overplot)) then begin
          if n_elements(position) eq 0 then begin
            plot,[x_b0,x_b1],[y_b1,y_b0],/nodata,xst=5,yst=5, $
              _EXTRA = extra,position=position,/norm
          endif else begin
            plot,[x_b0,x_b1],[y_b1,y_b0],/nodata,xst=5,yst=5, $
              _EXTRA = extra,position=position,/norm
          endelse
...
;
        r = 10.                          ;len of arrow head
        angle = 30. * !dtor            ;Angle of arrowhead
        st = r * sin(angle)             ;sin 22.5 degs * length of head
        ct = r * cos(angle)
;
        for i=0L,n_elements(good)-1 do begin     ;Each point
                if mag[i] lt 0.4*cs then scale=0.4*cs/mag[i] else scale=1.
                ...
                dx= dx*maxmag/r1
                dy= dy*maxmag/r1
;                st2=st*mag[i]/maxmag*scale
;                ct2=ct*mag[i]/maxmag*scale
                st2=st
                ct2=ct
                plots,[x0,x1,x1-(ct2*(dx/xd)-st2*(dy/xd))*xd, $
                      x1,x1-(ct2*(dx/xd)+st2*(dy/xd))*xd], $
                      [y0,y1,y1-(ct2*(dy/yd)+st2*(dx/yd))*yd, $
                      y1,y1-(ct2*(dy/yd)-st2*(dx/yd))*yd], $
                      clip=clip,noclip=noclip,_EXTRA=extra
;                plots,[x0,x1,x1-(ct*headx-st*heady), $
;                      x1,x1-(ct*(headx)+st*(heady))], $
;                      [y0,y1,y1-(ct*(heady)+st*(headx)), $
;                      y1,y1-(ct*(heady)-st*(headx))], $
;                      clip=clip,noclip=noclip,_EXTRA=extra
        endfor
        x0=min(x)
        y0=max(y)+y_step*1.5
        cs=7/260.
        sina = length * (cs/maxmag)
        cosa = length * (0./maxmag)
        dx=sina
        dy=cosa
        x1=x0+dx
        y1=y0+dy
                ...
                dx= dx*maxmag/r1
                dy= dy*maxmag/r1
                plots,[x0,x1,x1-(ct*(dx/xd)-st*(dy/xd))*xd, $
                      x1,x1-(ct*(dx/xd)+st*(dy/xd))*xd], $
                      [y0,y1,y1-(ct*(dy/yd)+st*(dx/yd))*yd, $
                      y1,y1-(ct*(dy/yd)-st*(dx/yd))*yd], $
                      clip=clip,noclip=noclip,_EXTRA=extra
